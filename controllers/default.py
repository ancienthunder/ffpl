...
def results():
    ...
    for i in xrange(0, 10, 2):
        for j in xrange(0, len(cur_gameweek[i]), 2):
            team_h.append([TD(cur_gameweek[i][j]), TD(cur_gameweek[i][j+1])])
            team_a.append([TD(cur_gameweek[i+1][j]), TD(cur_gameweek[i+1][j+1])])
        tables.append(TABLE(TR(team_h[k][0],team_a[k][0]) for k in range(len(team_h))))
    #raise HTTP(200, str(cur_gameweek))
    return dict(message='Results for this gameweek!', res=table, raw_res=cur_gameweek)
